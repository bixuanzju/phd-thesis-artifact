...
Module terminating.
  Definition t (P : relation) (e1 e2 : exp) :=
...
Module candidate.
  Definition wf (S : relation) := forall e1 e2, S e1 e2 -> value e1 /\ value e2.
End candidate.
...
Proof with eauto; try omega.
  ...
  - Case "A = nat".
    introv TB.
    gen v1 v2.
    induction TB; introv TyA TyB VH; simp V in *; try solve [inverts~ VH].
...
Proof with simpl_env; eauto using subtype_well_type, mtsubst_swft, preservation_multi_step.
  ...
  - Case "S-forall".
    ...
    apply sub_change with (Δ' := d) in Sub...
    lets (WFC' & WFC) : sub_swft Sub'.

    forwards : rel_d_same PH...
    forwards Sub2 : mtsubst_sub p Sub'...
    lets (? & Temp) : sub_swft Sub2...
    autorewrite with lr_rewrite in *.
...
Proof with eauto using swfe_wfe, mtsubst_swft.
  introv Wft.
  gen x A.
