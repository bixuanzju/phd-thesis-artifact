...
(* Notation "Δ ⊢ A <: B ~~> co " := (sub Δ A B co) (at level 45). *)
...
Lemma wfe_weaken : forall T E F G,
  wfe (G ++ E) T ->
  uniq (G ++ F ++ E) ->
  wfe (G ++ F ++ E) T.
Proof with simpl_env; eauto.
...
Lemma typing_tvar_weakening : forall E F G e T D,
  typ (G ++ E) D  e T ->
  uniq (G ++ F ++ E) ->
  typ (G ++ F ++ E) D e T.
...
Inductive inter_tvar : sty -> var -> Prop :=
| inter_tvar_refl : forall X,
    inter_tvar (sty_var_f X) X 
| inter_tvar_and_l : forall A B X,
    inter_tvar A X ->
    inter_tvar (sty_and A B) X
| inter_tvar_and_r : forall A B X,
    inter_tvar B X ->
    inter_tvar (sty_and A B) X.

Lemma sub_inter_tvar : forall A B Δ c X,
    sub Δ A B c ->
    inter_tvar B X ->
    inter_tvar A X.
Proof with auto.
  introv SUB BVAR.
  inductions SUB; auto;
    try (solve [inversions BVAR]).

  inversions BVAR...

  apply inter_tvar_and_l...
  apply inter_tvar_and_r...
Qed.

Lemma tvar_in_inter_tvar: forall X A,
    inter_tvar A X ->
    X `in` fv_sty_in_sty A.
Proof with auto.
  introv AVAR. inductions AVAR; simpl; try fsetdec.
Qed.

Lemma sub_tvar : forall A X Δ c,
    sub Δ A (sty_var_f X) c ->
    X `in` fv_sty_in_sty A.
Proof with eauto.
  introv Sub.
  assert (I1: inter_tvar (sty_var_f X) X).
    apply inter_tvar_refl.
  forwards : sub_inter_tvar Sub I1.
  apply tvar_in_inter_tvar...
Qed.
...
Definition getR (p : list (atom * (sty * sty * relation))) : list (atom * relation) :=
  map (fun t => match t with | (a, b, R) => R end) p.

Definition getOne (p : list (atom * (sty * sty * relation))) : list (atom * sty) :=
  map (fun t => match t with | (a, b, R) => a end) p.

Definition getTwo (p : list (atom * (sty * sty * relation))) : list (atom * sty) :=
...
Lemma typing_rename : forall x y E t U T G,
    ...
    x `notin` dom E \u fv_exp_in_exp t ->
    y `notin` dom E \u fv_exp_in_exp t ->
